...
USES {$ifdef UNIX}cmem,cthreads,{$endif}
     myFiles,myPics,gl,glext,glut,sysutils,dateutils,math,complex{$ifdef Windows},windows{$endif},Process,cmdLineParseUtil;
CONST
...
VAR numberOfCPUs:longint=2;                   //number of CPUs used
    ...
    darts:T_Darts;                                 //anti aliasing positions
...
//    palpha:=alpha/quality;
    ...
    for iy:=0 to renderImage.height-1 do if (iy mod numberOfCPUs=plongint(p)^) then begin
      a:=renderScaler.transform(iy+darts[previewLevel,0],0).re;
      ...
      for kx:=1 to samples do begin
        ...
        for k:=(maxDepth shr 1)+1 to maxDepth do begin
          iterationStep;
          renderImage.incPixel(round(renderScaler.mrofsnart(0,x).im+darts[previewLevel,1]),iy,white*pAlpha);
        end;
      end;
    end;
...
PROCEDURE keyboard(key:byte; x,y:longint); cdecl;
  ...
  begin
    ...
    if viewState<4 then case key of
      ...
      23: //Strg+W
          begin killRendering; darts.destroy; renderImage.destroy; currImage.destroy; halt; end;
      43: //+
          begin
