...
PROCEDURE testAntiAlias(subcomplex:longint; name:string; translateX,translateY,rotate,distToEye:single);
  ...
  begin
    ...
    for i:=0 to numberOfSamples-1 do begin
      if i<subcomplex then complexity:=10
                      else complexity:=9;
...
begin
  ...
  phase:=1; complexity:=10;
  i:=0;                 if not(fileExists('symmetry'+twoDigit(i)+'b.vraw')) then testAntiAlias(7-49+i,'symmetry'+twoDigit(i)+'b.vraw',2/3,0,1/3,0.2*zoom[i]);
  for i:=49 downto 1 do if not(fileExists('symmetry'+twoDigit(i)+'b.vraw')) then testAntiAlias(7-49+i,'symmetry'+twoDigit(i)+'b.vraw',2/3,0,1/3,0.2*zoom[i]);
