...
VAR //***RESOLUTION DEPENDENT VALUES***
    ...
    viewScaler:T_Scaler;
    shinePic,pic,aidPic: T_floatMap;
...
VAR xChunk,yChunk:T_Chunk;
  ...
  PROCEDURE shineCollect;
    VAR i :longint;
        ps:P_floatColor;
    begin
      ps:=shinePic.rawData;
      for i:=0 to pic.size-1 do ps[i]:=ps[i]+getOverbright(picpointer[i]);
    end;

  PROCEDURE shineExecute;
    VAR i,k :longint;
        c   :T_floatColor;
        pt,ps  :P_floatColor;
        hist:array[-128..128] of longint;
        blurSigma:single;
        brightnessFactor:single;
    begin
      ps:=shinePic.rawData;
      pt:=     pic.rawData;
      for i:=-128 to 128 do hist[i]:=0;
      brightnessFactor:=1/aaSamples;
      for i:=0 to pic.size-1 do begin
        c:=ps[i]*brightnessFactor;
        ps[i]:=c;
        blurSigma:=system.ln(max(0.00197,(c[0]+c[1]+c[2])/3))/system.ln(1.05); // >=-127.68
        inc(hist[min(128,round(blurSigma))]);
      end;
      if hist[-128]<0.99*xRes*yRes then begin
        k:=hist[-128]; i:=-128;
        while k<0.90*xres*yres do begin inc(i); k:=k+hist[i]; end; brightnessFactor:=system.exp(system.ln(1.05)*i  );
        while k<0.99*xres*yres do begin inc(i); k:=k+hist[i]; end; blurSigma       :=system.exp(system.ln(1.05)*i/2)*0.01;
        if brightnessFactor>1 then for i:=0 to pic.size-1 do ps[i]:=ps[i]*(1/brightnessFactor);
        blurImage(shinePic,blurSigma*sqrt(xres*xres+yres*yres));
        if brightnessFactor>1 then for i:=0 to pic.size-1 do pt[i]:=projectedColor(pt[i]+ps[i]*brightnessFactor)
                              else for i:=0 to pic.size-1 do pt[i]:=projectedColor(pt[i]+ps[i]);
      end;
    end;
  ...
  begin
    ...
    if picReady<aaSamples then begin
      ...
      if picReady=0 then begin
        pt:=shinePic.rawData; for i:=0 to shinePic.size-1 do pt[i]:=black;
      ...
      flushChunk;
      if performShining then shineCollect;
      {$ifndef jobberMode}
      ...
      if (picReady=aaSamples) then begin
        picReady:=64;
        if performShining then shineExecute;
      end;
